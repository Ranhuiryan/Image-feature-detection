 ...
 * Energy, Contrast, Correlation, Entropy.
 *
 * Arguments:
...
void GLCM::CalcuOneTextureEValue(Mat src, TextureEValues& EValue, bool ToCheckMat)
{
    ...
    for(int i = 0; i < src.rows; i++)
        for(int j = 0; j < src.cols; j++)
        {
			...
			EValue.idMoment += (src.at<float>(j, i) / (1 + (double)(i - j)*(double)(i - j)));
        }

	double ux, uy, localtotal;
	ux = localtotal = uy = 0.0;
	for (int i = 0; i < src.rows; i++)
	{
		localtotal = 0.0;
		for (int j = 0; j < src.cols; j++)
		{
			localtotal += src.at<float>(i, j);
		}
	...
	if (sigmax != 0 && sigmay != 0)
	{
		for (int i = 0; i < src.rows; i++)
		{
			for (int j = 0; j < src.cols; j++)
			{
				EValue.correlation = ((double)i * (double)j * src.at<float>(i, j)) / (sigmax * sigmay);
			}
		}
		EValue.correlation = EValue.correlation - (ux * uy) / (sigmax * sigmay);
	}
	else
		EValue.correlation = NULL;
}


/*===================================================================
 * 函数名：CalcuTextureEValue
 * 说明：计算全图的图像纹理特征值，包括能量、对比度、相关度、熵、逆差矩
 * 参数：
 *   Mat src:  源矩阵，窗口矩阵
...
void GLCM::CalcuTextureEValue(Mat src, TextureEValues& EValue, int size, GrayLevel level)
{
    ...
    EValue.contrast = 0; EValue.energy = 0; EValue.entropy = 0; EValue.correlation = 0; EValue.idMoment = 0;
    ...
    for(int i = 0; i < imgGray.rows; i++)
    {
        for(int j = 0; j < imgGray.cols; j++)
        {
            ...
            float energy, contrast, correlation, entropy, idMoment;
            energy = contrast = correlation = entropy = idMoment = 0;
			...
			correlation += EValue_temp.correlation; entropy += EValue_temp.entropy; idMoment += EValue_temp.idMoment;
            ...
            energy /= 4; contrast /= 4;
			correlation /= 4; entropy /= 4; idMoment /= 4;
            ...
            EValue.correlation += correlation;
			EValue.idMoment += idMoment;
        }
    }
}
 ...
 *   Mat& imgCorrelation - Destination Mat, Correlation Matrix
...
=====================================================================
*/
void GLCM::CalcuTextureImages(Mat src, Mat& imgEnergy, Mat& imgContrast, Mat& imgCorrelation, Mat& imgEntropy, Mat& imgidMoment, 
                        int size, GrayLevel level, bool ToAdjustImg)
{
    ...
    imgCorrelation.create(src.size(), CV_32FC1);
    imgEntropy.create(src.size(), CV_32FC1);
	imgidMoment.create(src.size(), CV_32FC1);

    for(int i = 0; i < src.rows; i++)
    {
        float* energyData = imgEnergy.ptr<float>(i);
        float* contrastData = imgContrast.ptr<float>(i);
        float* correlationData = imgCorrelation.ptr<float>(i);
        float* entropyData = imgEntropy.ptr<float>(i);
		float* idMomentData = imgidMoment.ptr<float>(i);
        ...
        for(int j = 0; j < src.cols; j++)
        {
            ...
            correlation += EValue.correlation; entropy += EValue.entropy; idMoment += EValue.idMoment;
            ...
            correlationData[j] = correlation;
            entropyData[j] = entropy;
			idMomentData[j] = idMoment;
        }
    }
    ...
    if(ToAdjustImg)
    {
        ...
        cv::normalize(imgCorrelation, imgCorrelation, 0, 255, NORM_MINMAX);
		cv::normalize(imgidMoment, imgidMoment, 0, 255, NORM_MINMAX);
        imgEnergy.convertTo(imgEnergy, CV_8UC1);
        imgContrast.convertTo(imgContrast, CV_8UC1);
        imgEntropy.convertTo(imgEntropy, CV_8UC1);
        imgCorrelation.convertTo(imgCorrelation, CV_8UC1);
		imgidMoment.convertTo(imgidMoment, CV_8UC1);
    }
}
